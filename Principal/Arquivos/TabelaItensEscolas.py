...
def gerarTabelaItensEscolas(ano): # Gera uma tabela para correlacionar com algum índice
    dfEscolas = pd.read_csv('../Dados/escolas'+ano+'.csv',low_memory=False, encoding='ISO-8859-1', sep='|')
...
def gerarTabelaQtdAlunosPorEscola(ano): # Gera uma tabela com a qtd de alunos por escola
    dfTurmas = pd.read_csv('../Dados/turmas'+ano+'.csv',low_memory=False, encoding='ISO-8859-1', sep='|')
...
def mergeTabelas(ano): # Faz o merge de ItensEscolas com QtdAlunosPorEscola
    itensEscolas = pd.read_csv('../Dados/Correlacao/itensPorEscolas'+ano+'_out.csv',low_memory=False, encoding='ISO-8859-1', sep='|')
    qtdAlunosEscola = pd.read_csv('../Dados/Correlacao/qtdAlunosPorEscola'+ano+'_out.csv',low_memory=False, encoding='ISO-8859-1', sep='|')
...
def agruparMunicipio(ano,tabela):  #Tabela para correlacionar com IOEB 
    ...
    tabelaItens_Municipio.to_csv('../Dados/Correlacao/itens_Por_Municipio'+ano+'_out.csv','|')
